...
따라서 제품의 본체는 링크 목록이 아니라 **Bridge Agent 연결 그래프**다.
...
> Bridge Agent는 학술 자료와 산업 자료를 각각 요약하는 도구가 아니라, `기술·사용 사례·환경·기대 효과` 단위로 두 세계를 연결하고, 연결된 부분·끊긴 부분·중단된 부분·잠재적으로 연결 가능한 부분을 근거와 함께 판정하는 리서치 에이전트다.
