    ...
    def __init__(self, *, small_model: str = SMALL_MODEL, large_model: str = LARGE_MODEL):
        ...
        self.final_synthesis_writer = Agent(
            ...
            instructions=(
                ...
                "'추정' 또는 '확인되지 않음'으로 분리한다. adoption_evidence가 0이거나 adoption_evidence "
                "목록이 비어 있으면 산업 도입이 확인됐다고 쓰지 않는다. label이 no_gap이거나 gap_types가 "
                "비어 있으면 '간극 원인'에서는 확인된 간극 원인이 없다고 쓰고, candidate_connections의 "
                "missing dimension을 도입 장벽처럼 해석하지 않는다. 한국어로 작성한다.\n"
